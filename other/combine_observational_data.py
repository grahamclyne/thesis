...
import hydra
from other.utils import readCoordinates,getRollingWindow
import numpy as np
...
def main(cfg: DictConfig):
    ...
    era_data = pd.read_csv(f'{cfg.path.data}/era_data.csv',index_col=False)
    nfis_data = pd.read_csv(f'{cfg.path.data}/generated_data/nfis_tree_cover_data.csv')
    ...
    final_input.to_csv(f'{cfg.path.data}/cleaned_observed_ann_input.csv',index=False)
    ...
    test_data.to_csv(f'{cfg.path.data}/{cfg.files.output}',index=False)
