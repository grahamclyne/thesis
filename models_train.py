...
def get_training_data(cfg,run):
    ...
    train_ds,val_ds = split_data(merged)
    ...
    return train_ldr,validation_ldr,train_ds,val_ds
...
def main(cfg: DictConfig):
    ...
    train_loader,validation_loader,train_ds,val_ds = get_training_data(cfg,run)
    ...
    for epoch_count in range(cfg.model.epochs):
        ...
        wandb.log({'train_r2_score':np.corrcoef(pred_y,tgt)})
        ...
        wandb.log({'validation_r2_score':np.corrcoef(pred_y,tgt)})
    ...
    model_feature.fit(train_ds[:,cfg.model.input],train_ds[:,cfg.model.output])
    pred = model_feature.predict(val_ds[:,cfg.model.input])
    print(np.corrcoef(pred,val_ds[:,cfg.model.output]))
    lamse = lat_adjusted_mse(pred,val_ds[:,cfg.model.output],val_ds[:,cfg.model.id][:,1])
    print(lamse)
    print(np.sum(lamse))
